...
def generator(b_s, phase_gen="train"):
    if phase_gen == "train":
        ...
        images = [
            imgs_val_path + f for f in os.listdir(imgs_val_path) if f.endswith(".jpg")
        ]
        maps = [
            maps_val_path + f for f in os.listdir(maps_val_path) if f.endswith(".jpg")
        ]
    else:
    ...
    while True:
        yield preprocess_images(
            images[counter : counter + b_s], shape_r, shape_c
        ), preprocess_maps(maps[counter : counter + b_s], shape_r_gt, shape_c_gt)
        counter = (counter + b_s) % len(images)


def generator_test(b_s, imgs_test_path):
    images = [
        imgs_test_path + f for f in os.listdir(imgs_test_path) if f.endswith(".jpg")
    ]
    images.sort()
    ...
    if phase == "train":
        ...
        model.fit_generator(
            ...
            callbacks=[
                ...
                ModelCheckpoint(
                    "weights.mlnet.{epoch:02d}-{val_loss:.4f}.pkl", save_best_only=True
                ),
            ],
        )
