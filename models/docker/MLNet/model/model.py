...
def ml_net_model(
    ...
    conv1_1 = Convolution2D(
        64, 3, 3, weights=weights, activation="relu", border_mode="same"
    )(input_ml_net)
    ...
    conv2_1 = Convolution2D(
        128, 3, 3, weights=weights, activation="relu", border_mode="same"
    )(conv1_pool)
    ...
    conv3_1 = Convolution2D(
        256, 3, 3, weights=weights, activation="relu", border_mode="same"
    )(conv2_pool)
    ...
    conv4_1 = Convolution2D(
        512, 3, 3, weights=weights, activation="relu", border_mode="same"
    )(conv3_pool)
    ...
    #########################################################
    concatenated = merge(
        [conv3_pool, conv4_pool, conv5_3], mode="concat", concat_axis=1
    )
    dropout = Dropout(0.5)(concatenated)

    int_conv = Convolution2D(
        64, 3, 3, init="glorot_normal", activation="relu", border_mode="same"
    )(dropout)

    pre_final_conv = Convolution2D(1, 1, 1, init="glorot_normal", activation="relu")(
        int_conv
    )
    ...
    #########################################################
    rows_elt = (
        math.ceil(img_rows / downsampling_factor_net) // downsampling_factor_product
    )
    cols_elt = (
        math.ceil(img_cols / downsampling_factor_net) // downsampling_factor_product
    )
    eltprod = EltWiseProduct(init="zero", W_regularizer=l2(1 / (rows_elt * cols_elt)))(
        pre_final_conv
    )
...
def loss(y_true, y_pred):
    max_y = K.repeat_elements(
        K.expand_dims(
            K.repeat_elements(
                K.expand_dims(K.max(K.max(y_pred, axis=2), axis=2)), shape_r_gt, axis=-1
            )
        ),
        shape_c_gt,
        axis=-1,
