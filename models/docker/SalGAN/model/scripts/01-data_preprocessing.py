...
listImgFiles = [
    k.split("/")[-1].split(".")[0] for k in glob.glob(os.path.join(pathToMaps, "*"))
]
listTestImages = [
    k.split("/")[-1].split(".")[0]
    for k in glob.glob(os.path.join(pathToImages, "*test*"))
]

for currFile in tqdm(listImgFiles):
    tt = dataRepresentation.Target(
        os.path.join(pathToImages, currFile + ".jpg"),
        ...
        dataRepresentation.LoadState.loaded,
        dataRepresentation.InputType.saliencyMapMatlab,
    ...
    imageResized = cv2.cvtColor(
        cv2.resize(tt.image.getImage(), img_size, interpolation=cv2.INTER_AREA),
        cv2.COLOR_RGB2BGR,
    )
    saliencyResized = cv2.resize(
        tt.saliency.getImage(), salmap_size, interpolation=cv2.INTER_AREA
    )
...
for currFile in tqdm(listFilesTrain):
    trainData.append(
        dataRepresentation.Target(
            os.path.join(pathOutputImages, currFile + ".png"),
            os.path.join(pathOutputMaps, currFile + ".png"),
            os.path.join(pathToFixationMaps, currFile + ".mat"),
            dataRepresentation.LoadState.loaded,
            dataRepresentation.InputType.image,
            dataRepresentation.LoadState.loaded,
            dataRepresentation.InputType.imageGrayscale,
            dataRepresentation.LoadState.loaded,
            dataRepresentation.InputType.fixationMapMatlab,
        )
    )
