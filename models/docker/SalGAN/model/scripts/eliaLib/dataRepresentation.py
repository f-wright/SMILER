...
class ImageContainer:
    ...
    def load(self):
        ...
        if self.imageType == InputType.image:
            self.data = cv2.cvtColor(
                cv2.imread(self.filePath, cv2.IMREAD_COLOR), cv2.COLOR_BGR2RGB
            )
            self.state = LoadState.loaded
        if self.imageType == InputType.imageGrayscale:
            self.data = cv2.cvtColor(
                cv2.imread(self.filePath, cv2.IMREAD_COLOR), cv2.COLOR_BGR2GRAY
            )
            self.state = LoadState.loaded
    ...
    def loadCompressed(self):
        ...
        if self.imageType == InputType.image:
            ...
            raise Exception(
                "Saliency maps do no have compressed handlind method enabled"
            )
        elif self.imageType == InputType.empty:
            self.state = LoadState.error
            raise Exception(
                "Empty images do no have compressed handlind method enabled"
            )
    ...
    def getImage(self):
        ...
        if self.imageType == InputType.image:
            if self.state == LoadState.unloaded:
                ...
                return cv2.cvtColor(
                    cv2.imdecode(self.data, cv2.IMREAD_COLOR), cv2.COLOR_BGR2RGB
                )
            ...
            if self.state == LoadState.unloaded:
                ...
                return cv2.cvtColor(
                    cv2.imdecode(self.data, cv2.IMREAD_COLOR), cv2.COLOR_BGR2GRAY
                )
        elif self.imageType == InputType.saliencyMapMatlab:
            if self.state == LoadState.unloaded:
            ...
            if self.state == LoadState.unloaded:
                ...
                raise Exception(
                    "Fixation maps do no have compressed handlind method enabled"
                )
                return None
        elif self.imageType == InputType.empty:
            return None
...
class Target:
    def __init__(
        ...
        saliencyPath,
        fixationPath,
        imageState=LoadState.unloaded,
        imageType=InputType.image,
        saliencyState=LoadState.unloaded,
        saliencyType=InputType.saliencyMapMatlab,
        fixationState=LoadState.unloaded,
        fixationType=InputType.fixationMapMatlab,
    ):
        self.image = ImageContainer(imagePath, imageType, imageState)
        self.saliency = ImageContainer(saliencyPath, saliencyType, saliencyState)
