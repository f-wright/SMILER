...
class ModelSALGAN(Model):
    def __init__(self, w, h, batch_size=32, G_lr=3e-4, D_lr=3e-4, alpha=1 / 20.0):
        ...
        output_var_pooled = T.signal.pool.pool_2d(
            self.output_var, (4, 4), mode="average_exc_pad", ignore_border=True
        )
        prediction_pooled = T.signal.pool.pool_2d(
            prediction, (4, 4), mode="average_exc_pad", ignore_border=True
        )
        train_err = lasagne.objectives.binary_crossentropy(
            prediction_pooled, output_var_pooled
        ).mean()
        ...
        D_obj = lasagne.objectives.binary_crossentropy(
            ...
            T.concatenate([ones, zeros], axis=0),
        ).mean()
        +1e-4 * lasagne.regularization.regularize_network_params(
        ...
        G_obj_d = lasagne.objectives.binary_crossentropy(
            disc_gen, T.ones(disc_lab.shape)
        ).mean()
        +1e-4 * lasagne.regularization.regularize_network_params(
            self.net[output_layer_name], lasagne.regularization.l2
        )
        ...
        G_params = lasagne.layers.get_all_params(
            self.net[output_layer_name], trainable=True
        )
        ...
        D_params = lasagne.layers.get_all_params(
            self.discriminator["prob"], trainable=True
        )
        ...
        self.D_trainFunction = theano.function(
            [self.input_var, self.output_var],
            cost,
            updates=D_updates,
            allow_input_downcast=True,
        )
