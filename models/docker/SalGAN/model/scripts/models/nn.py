...
def centered_softplus(x):
    return T.nnet.softplus(x) - np.cast[th.config.floatX](np.log(2.0))
...
def adam_updates(params, cost, lr=0.001, mom1=0.9, mom2=0.999):
    ...
    t = th.shared(np.cast[th.config.floatX](1.0))
    for p, g in zip(params, grads):
        v = th.shared(np.cast[th.config.floatX](p.get_value() * 0.0))
        mg = th.shared(np.cast[th.config.floatX](p.get_value() * 0.0))
        v_t = mom1 * v + (1.0 - mom1) * g
        mg_t = mom2 * mg + (1.0 - mom2) * T.square(g)
        v_hat = v_t / (1.0 - mom1**t)
        mg_hat = mg_t / (1.0 - mom2**t)
...
class WeightNormLayer(lasagne.layers.Layer):
    def __init__(
        ...
        **kwargs
        ...
        if g is not None:
            self.g = self.add_param(
                g, (k,), name="g", regularizable=False, trainable=train_g
            )
        ...
        if g is not None:
            incoming.W = incoming.W_param * (
                self.g
                / T.sqrt(1e-6 + T.sum(T.square(incoming.W_param), axis=W_axes_to_sum))
            ).dimshuffle(*W_dimshuffle_args)
        else:
            incoming.W = incoming.W_param / T.sqrt(
                1e-6
                + T.sum(T.square(incoming.W_param), axis=W_axes_to_sum, keepdims=True)
            )
    ...
    def get_output_for(self, input, init=False, **kwargs):
        if init:
            ...
            inv_stdv = self.init_stdv / T.sqrt(
                T.mean(T.square(input), self.axes_to_sum)
            )
...
class MinibatchLayer(lasagne.layers.Layer):
    def __init__(
        ...
        num_kernels,
        dim_per_kernel=5,
        theta=lasagne.init.Normal(0.05),
        log_weight_scale=lasagne.init.Constant(0.0),
        ...
        **kwargs
        ...
        self.theta = self.add_param(
            theta, (num_inputs, num_kernels, dim_per_kernel), name="theta"
        )
        self.log_weight_scale = self.add_param(
            log_weight_scale, (num_kernels, dim_per_kernel), name="log_weight_scale"
        )
        self.W = self.theta * (
            T.exp(self.log_weight_scale) / T.sqrt(T.sum(T.square(self.theta), axis=0))
        ).dimshuffle("x", 0, 1)
        ...
        if init:
            ...
            self.init_updates = [
                (
                    ...
                    self.log_weight_scale - T.log(mean_min_abs_dif).dimshuffle(0, "x"),
                )
            ]
...
class BatchNormLayer(lasagne.layers.Layer):
    def __init__(
        ...
        **kwargs
        ...
        self.avg_batch_mean = self.add_param(
            lasagne.init.Constant(0.0),
            (k,),
            name="avg_batch_mean",
            regularizable=False,
            trainable=False,
        )
        self.avg_batch_var = self.add_param(
            lasagne.init.Constant(1.0),
            (k,),
            name="avg_batch_var",
            regularizable=False,
            trainable=False,
    ...
    def get_output_for(self, input, deterministic=False, **kwargs):
        if deterministic:
            ...
            batch_var = T.mean(
                T.square(centered_input), axis=self.axes_to_sum
            ).flatten()
            ...
            new_v = (
                0.9 * self.avg_batch_var
                + T.cast(
                    (0.1 * input.shape[0]) / (input.shape[0] - 1), th.config.floatX
                )
                * batch_var
            )
            self.bn_updates = [
                (self.avg_batch_mean, new_m),
...
def batch_norm(
    layer, b=lasagne.init.Constant(0.0), g=lasagne.init.Constant(1.0), **kwargs
):
    """
...
def l2normalize(layer, train_scale=True):
    ...
    layer.W = W_param * (
        layer.W_scale / T.sqrt(1e-6 + T.sum(T.square(W_param), axis=axes_to_sum))
    ).dimshuffle(*dimshuffle_args)
    return layer
...
class DenseLayer(lasagne.layers.Layer):
    def __init__(
        ...
        b=lasagne.init.Constant(0.0),
        weight_scale=lasagne.init.Constant(1.0),
        train_scale=False,
        nonlinearity=relu,
        **kwargs
    ):
        ...
        self.weight_scale = self.add_param(
            weight_scale, (num_units,), name="weight_scale", trainable=train_scale
        )
        self.W = self.theta * (
            self.weight_scale / T.sqrt(T.sum(T.square(self.theta), axis=0))
        ).dimshuffle("x", 0)
    ...
    def get_output_for(self, input, init=False, deterministic=False, **kwargs):
        ...
        if init:
            ...
            self.init_updates = [
                (self.weight_scale, self.weight_scale / stdv),
                (self.b, -ma / stdv),
            ]
        else:
