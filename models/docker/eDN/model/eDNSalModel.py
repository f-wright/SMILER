...
class EDNSaliencyModel(object):
    def __init__(self, descriptions, svmModel, biasToCntr=False):
        ...
        if diff != 0:
            if diff == 1 and not biasToCntr:
                raise ValueError(
                    "The number of features in eDN and svm "
    ...
    def saliency(self, img, normalize=True):
        ...
        imgSize = img.shape[:2]
        rescale_factor = 0.5 * EDN_INSIZE[0] / max(imgSize) + 0.5 * EDN_INSIZE[1] / min(
            imgSize
        )
        # single image:
        ...
        predLarge = sp.ndimage.interpolation.zoom(
            pred, (imgSize[0] / float(pred.shape[0]), imgSize[1] / float(pred.shape[1]))
        )
        ...
        if normalize:
            rescaled = (
                255.0
                / (predLarge.max() - predLarge.min())
                * (predLarge - predLarge.min())
            ).astype(np.uint8)
            return rescaled
        else:
            return predLarge
