    ...
    for i in xrange(len(desc)):
        ...
        if desc[i]["colorSp"] == "yuv":
            ...
            for j in xrange(imgC.shape[2] / 3):
                imgC[:, :, j * 3 : j * 3 + 3] = convertRGB2YUV(
                    imgC[:, :, j * 3 : j * 3 + 3]
                )

        imgC = imgC.astype("f")

        model = SequentialLayeredModel((imgC.shape[0], imgC.shape[1]), desc[i]["desc"])
...
def dist_to_cntr_features(img, outSize=None):
    ...
    for x in xrange(imgSize[0]):
        for y in xrange(imgSize[1]):
            distMat[x, y] = np.floor(
                np.sqrt((x - midpointx) ** 2 + (y - midpointy) ** 2)
            )
