...
if __name__ == "__main__":
    ...
    nFeatures = np.sum(
        [d["desc"][-1][0][1]["initialize"]["n_filters"] for d in desc if d != None]
    )
    ...
    with open(whiten_path) as fp:
        whitenParams = np.asarray([map(float, line.split(" ")) for line in fp]).T
    ...
    def eDNsaliency(image_path):
        ...
        normSalMap = (
            255.0 / (salMap.max() - salMap.min()) * (salMap - salMap.min())
        ).astype(np.uint8)
