...
class Salicon:
    def __init__(
        self, prototxtpath="salicon.prototxt", model="salicon_osie.caffemodel"
    ):
    ...
    def compute_saliency(self, image_path):
        im = np.array(
            Image.open(image_path), dtype=np.float32
        )  # in RGB - ASSUMING 256 IMAGE
        ...
        sal_map = scipy.ndimage.interpolation.zoom(
            sal_map,
            tuple(
                np.asarray(im.shape[2:], dtype=np.float32)
                / np.asarray(sal_map.shape, dtype=np.float32)
            ),
            np.dtype(np.float32),
            mode="nearest",
        )
