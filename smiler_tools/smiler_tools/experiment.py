...
class Experiment(object):
    ...
    def _realpath_relative_to_yaml(self, file_path_relative_to_yaml):
        if self._yaml_path:
            yaml_path_dirname = os.path.dirname(os.path.realpath(self._yaml_path))
            file_path_real = os.path.join(yaml_path_dirname, file_path_relative_to_yaml)
    ...
    def set_from_yaml(self, yaml_fp):
        ...
        self._input_path = self._realpath_relative_to_yaml(experiment_map["input_path"])

        self._base_output_path = self._realpath_relative_to_yaml(
            experiment_map["base_output_path"]
        )

        for param_name, param_val in experiment_map.get("parameters", {}).items():
            self._experiment_parameter_map.set(param_name, param_val)

        for run_map in runs_list:
            for model in self._model_manager.get_matching(run_map["algorithm"]):

                output_path = self._realpath_relative_to_yaml(
                    run_map.get(
                        "output_path", os.path.join(self._base_output_path, model.name)
                    )
                )
    ...
    def set_from_models_string(self, models_string, input_path, base_output_path):
        ...
        for selected_model in selected_models:
            ...
            run = ExperimentRun(
                selected_model, input_path, output_path, self._config_parameter_map
            )
            self._runs.append(run)
...
class ExperimentRun(object):
    ...
    def run(self, experiment_parameter_map):
        ...
        self._model.run_batch(
            self._input_path, self._output_path, self._config_parameter_map, my_run_map
        )
