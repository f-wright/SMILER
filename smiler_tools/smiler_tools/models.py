...
class SMILERModel(object):
    ...
    def run_batch(
        self, input_dir, output_dir, config_parameter_map, experiment_parameter_map=None
    ):
...
class DockerModel(SMILERModel):
    def __init__(self, **kwargs):
        ...
        self.docker_image = "{}:{}".format(kwargs.get("docker_image"), self.version)
    ...
    def run_batch(
        ...
        model_run_command = [
            ...
            "-it",
            "--volume",
            "{}:/opt/model".format(model_dir),
            "--volume",
            "{}:/opt/input_vol".format(input_dir),
            "--volume",
            "{}:/opt/output_vol".format(output_dir),
            "--shm-size=128m",
            "-e",
            "SMILER_PARAMETER_MAP={}".format(json.dumps(parameter_map.get_pair_dict())),
            "--rm",
            self.docker_image,
        ] + self.run_command
    ...
    def run_batch(
        ...
        for img_number, path_tuple in enumerate(image_path_tuples):
            ...
            if not options_overwrite and os.path.exists(output_path):
                ...
                try:
                    print(
                        "Running image [{}/{}]: {}".format(
                            img_number + 1, num_paths, printable_input_path
                        )
                    )
                    img = matlab_engine.imread(input_path)

                    parameter_struct = parameter_map.get_matlab_struct(matlab_engine)
...
class ModelManager(object):
    ...
    def load_model(self, smiler_json_path):
        ...
        if model_type == "docker":
            ...
            msg = "Unknown model type {} in {}".format(model_type, smiler_json_path)
            print("ERROR: " + msg)
    ...
    def get_matching(self, model_names):
        ...
        for name in clean_names:
            if name in self.MODEL_COLLECTIONS:
                ...
                possible_models = [model.name for model in self._model_map.values()]
                possible_models.sort()
                possible_models = self.MODEL_COLLECTIONS.keys() + possible_models
    ...
    def get_model_collection(self, kind):
        if kind == "all":
            return [
                model for key, model in self._model_map.items() if not model.invariant
            ]
        elif kind == "docker":
            return [
                model
            ...
            return [model for key, model in self._model_map.items() if model.invariant]
        else:
