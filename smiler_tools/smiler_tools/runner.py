...
def run_model(
    compute_saliency, input_dir="/opt/input_vol/", output_dir="/opt/output_vol/"
):
    """
    Parameters:
    ...
    for img_number, path_tuple in enumerate(image_path_tuples):
        ...
        if not options_overwrite and os.path.isfile(output_path):
            ...
            post_processed_image = image_processing.post_process(saliency_map, options)

            image_processing.save_image(
                output_path, post_processed_image, uid=target_uid, gid=target_gid
            )
